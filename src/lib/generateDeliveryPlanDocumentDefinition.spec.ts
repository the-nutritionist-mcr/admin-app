...
const isTable = (thing: Content): thing is ContentTable => Boolean((thing as ContentTable).table)
...
describe("generate delivery plan document definition", () => {
  it("is sorted in alphabetical order", () => {
    ...
    const definition = generateDeliveryPlanDocumentDefinition([
      { customer: mockCustomer2, deliveries: ["paused", "paused"]},
      { customer: mockCustomer1, deliveries: ["paused", "paused"]},
      { customer: mockCustomer3, deliveries: ["paused", "paused"]},
      { customer: mockCustomer4, deliveries: ["paused", "paused"]}
    ], [])

    if(Array.isArray(definition.content)) {
      const table = definition.content.find(thing => isTable(thing) && thing.table)

      if(table && isTable(table)) {
        expect((table.table.body[0][0] as any).text[0].text.startsWith("abc")).toBeTruthy()
        expect((table.table.body[1][0] as any).text[0].text.startsWith("dcf")).toBeTruthy()
        expect((table.table.body[2][0] as any).text[0].text.startsWith("def")).toBeTruthy()
        expect((table.table.body[3][0] as any).text[0].text.startsWith("gef")).toBeTruthy()
      }
    }
