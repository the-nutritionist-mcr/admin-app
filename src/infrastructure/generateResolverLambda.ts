...
export const generateResolverLambda = (context: Construct, api: IGraphqlApi, name: string, type: ResolverType, environment: { [key: string]: string}) => {
  const bundlePath = process.env.IS_LOCAL_DEPLOY
    ? path.resolve(__dirname, "..", "..", "dist", "bundles", "backend", "handlers")
